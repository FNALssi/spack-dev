...
def extract_specs(spec_source):
    ...
    if type(spec_source) == list:
        ...
        with open(spec_source, 'r') as dag_file:
            spec_args.append(dag_file.read().rstrip())
    return specs.cmd.parse_specs(spec_args)
...
def init_new(parser, args):
    ...
    if args.base_dir:
        if os.path.isdir(args.base_dir) or \
           os.path.exists(os.path.dirname(args.base_dir)):
            ...
            tty.die('spackdev init_new: {0} is not a directory or its parent does not exist.'
    ...
    try:
        ...
        tty.die('spackdev init_new: unable to make or change directory to {0}'
    ...
    if os.listdir(spackdev_base):
        if args.force:
            tty.warn('spackdev init_new: (force) using non-empty directory {0}'
                     .format(spackdev_base))
        else:
            tty.die('spackdev init_new: refusing to use non-empty directory {0}'
                    .format(spackdev_base))

    if (os.path.exists('spackdev-aux')):
        if args.force:
            tty.warn('spackdev init_new: (force) removing existing spackdev-aux, build and install directories from {0}'
                     .format(spackdev_base))
            for wd in ('spackdev-aux', 'install', 'build'):
                shutil.rmtree(wd, ignore_errors=True)
        else:
            tty.die('spackdev init_new: cannot re-init (spackdev-aux directory exists)')
